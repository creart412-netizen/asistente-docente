...
import docx
from docx.shared import RGBColor # Importar para el color de la fuente en Word
import pandas as pd
import openai
import os
...
import re # Importar para expresiones regulares para parsear HTML
...
# from docx2pdf import convert as docx_to_pdf_convert # Ya no es necesario, lo removemos
...
def parse_and_format_math_problem(raw_text):
    ...
    for line in lines:
        ...
        # Handle cases where the line doesn't start with a keyword but is part of the previous section
        elif current_section == "steps" and line_stripped:
            if steps:
                steps[-1] += "\n" + line_stripped # Append to the last step if it's multi-line
            else:
                steps.append(line_stripped) # If it's the very first line of steps
...
def highlight_errors_in_text(text, matches):
    ...
    Improved to handle various newline characters and ensure correct offsetting.
    ...
    """
    if not matches:
        return text.replace('\n', '<br>').replace('\r', '') # Convert newlines for HTML display
    ...
    sorted_matches = sorted(matches, key=lambda x: x.offset + x.errorLength, reverse=True)

    highlighted_text_list = list(text) # Convert string to list of characters for easier manipulation
    ...
    for match in sorted_matches:
        ...
        if 0 <= start < len(highlighted_text_list) and 0 <= end <= len(highlighted_text_list):
            error_phrase = "".join(highlighted_text_list[start:end])
            # Escape HTML characters in the error phrase to prevent issues if it contains < or >
            escaped_error_phrase = error_phrase.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')
            highlighted_span = f'<span class="highlight-error">{escaped_error_phrase}</span>'
            highlighted_text_list[start:end] = list(highlighted_span)
        else:
            st.warning(f"Error al aplicar resaltado: Coordenadas de error fuera de rango para '{match.ruleId}' en '{match.sentence}'. Offset: {match.offset}, Length: {match.errorLength}. Longitud del texto: {len(text)}")


    return "".join(highlighted_text_list).replace('\n', '<br>').replace('\r', '')


def create_word_doc_from_highlighted_html(html_text, file_name_prefix="Documento_Revisado"):
    """
    Converts HTML text (with specific error highlights) into a Word document.
    Errors highlighted with <span class="highlight-error"> will be red in the DOCX.
    This version more carefully handles newlines within text parts.
    """
    doc = docx.Document()
    
    # Normalize newlines for consistent splitting
    text_with_newlines_normalized = html_text.replace('<br>', '\n').replace('\r', '')

    # Split the text by the highlight spans, capturing the spans themselves
    parts = re.split(r'(<span class="highlight-error">.*?</span>)', text_with_newlines_normalized, flags=re.DOTALL)

    current_paragraph = doc.add_paragraph()

    for part in parts:
        if part.startswith('<span class="highlight-error">') and part.endswith('</span>'):
            # This is a highlighted part
            match = re.search(r'<span class="highlight-error">(.*?)</span>', part, re.DOTALL)
            if match:
                error_content = match.group(1)
                # Ensure the run is added to the *current* paragraph
                run = current_paragraph.add_run(error_content)
                run.font.color.rgb = RGBColor(220, 53, 69) # Red color (DC3545 in hex)
        else:
            # This is normal text. It might contain multiple newlines.
            # We want to split by newlines, adding new paragraphs for each,
            # but preserve empty lines (which means empty paragraphs).
            sub_parts = part.split('\n')
            
            for i, sub_part in enumerate(sub_parts):
                if i > 0: # If it's not the first sub_part, it means there was a newline
                    # If the current paragraph is not empty (i.e., we added text to it),
                    # we should create a new paragraph for the next part.
                    # This prevents creating empty paragraphs for trailing newlines
                    # but respects actual empty lines in the middle.
                    if current_paragraph.text or current_paragraph.runs: # Check if current paragraph has content
                        current_paragraph = doc.add_paragraph()
                    else: # If current paragraph is empty (e.g., from an initial newline)
                        pass # Don't create new paragraph yet, just use this one
                
                if sub_part: # Add text if not empty
                    current_paragraph.add_run(sub_part)

    # Clean up: if the last part resulted in an empty paragraph, remove it (optional, but often desired)
    if not current_paragraph.text.strip() and not current_paragraph.runs:
        if len(doc.paragraphs) > 1: # Only remove if not the only paragraph
...
for i, tab_name in enumerate(tabs_names):
    with cols[i % num_columns]: # Usamos el módulo para ciclar entre las columnas
        ...
        # Apply custom CSS class for selected button (handled in the <style> block)
        button_class = "st-button-selected" if is_selected else ""
        
        # Streamlit's internal button styling might override, so we rely on global CSS to manage this
...
if st.session_state.selected_section == "✏️ Revisión Ortográfica":
    st.header("✏️ Revisión de Textos")
    st.write("Sube un archivo PDF o Word para que la IA revise y resalte errores ortográficos y gramaticales.")
    archivo = st.file_uploader("Sube un archivo PDF o Word", type=["pdf", "docx"], key="revision_uploader")
    ...
    if 'last_highlighted_html' not in st.session_state: # Nuevo estado para almacenar el HTML resaltado
        st.session_state.last_highlighted_html = ""
    # Nuevo estado para evitar duplicados en Mis Archivos al recalcular el DOCX
    if 'word_download_saved_to_local' not in st.session_state:
        st.session_state.word_download_saved_to_local = False
    ...
    def get_language_tool():
        try:
            # SOLUCIÓN FINAL DEFINITIVA: Usar LanguageToolPublicAPI
            # Esto usa la API pública de LanguageTool de la manera correcta.
            # No necesitamos especificar 'remote_server' si usamos esta clase.
            return language_tool_python.LanguageToolPublicAPI(language='es-ES')
        except language_tool_python.utils.LanguageToolError as lt_error:
            st.error(f"Error al inicializar LanguageTool: {lt_error}. Esto podría deberse a problemas de conexión o límites de la API pública. Por favor, **espera unos minutos e intenta de nuevo**, o considera ejecutar un servidor local de LanguageTool para un uso intensivo.")
            st.stop()
        except Exception as e:
            st.error(f"Error inesperado al inicializar LanguageTool: {e}. Asegúrate de tener conexión a internet.")
            st.stop()
    
    tool = get_language_tool()

    # Process uploaded file only if a new file is provided or if previous state is empty
    # This block now correctly triggers text extraction and a rerun when a new file is uploaded
    if archivo is not None:
        st.info(f"Archivo '{archivo.name}' cargado. Extrayendo texto...")
        texto_extraido = ""
        ...
        if archivo_name_lower.endswith(".pdf"):
            ...
            if not texto_extraido.strip():
                st.warning("No se encontró texto digital en el PDF, intentando OCR (esto puede tardar unos segundos a minutos para archivos grandes)...")
                try:
                    ...
                    st.error(f"Error al procesar el PDF con OCR: {e}. Asegúrate de que Tesseract esté bien instalado y configurado en tu entorno.")
                    texto_extraido = ""
        ...
        # Check if extracted text is different from last time to avoid unnecessary reruns
        if st.session_state.last_extracted_text != texto_extraido:
            st.session_state.last_extracted_text = texto_extraido
            st.session_state.last_found_matches = [] # Clear previous matches
            st.session_state.last_highlighted_html = "" # Clear previous highlighted HTML
            st.session_state.word_download_saved_to_local = False # Reset save flag
            st.rerun() # Rerun to display extracted text and prompt for review
        else:
            st.info("El texto extraído es el mismo que la última vez.")
    ...
    if st.session_state.last_extracted_text.strip():
        ...
        st.markdown(f"<div style='border: 1px solid #ddd; padding: 10px; border-radius: 5px; overflow-wrap: break-word; max-height: 200px; overflow-y: auto;'>{st.session_state.last_extracted_text.replace('\n', '<br>')}</div>", unsafe_allow_html=True)
        st.markdown("---")

        if st.button("🔍 Revisar Texto", key="revisar_texto_btn"):
            if not st.session_state.last_extracted_text.strip():
                st.warning("No hay texto para revisar. Por favor, sube un archivo con contenido.")
                st.session_state.last_highlighted_html = ""
                st.session_state.word_download_saved_to_local = False
                st.stop() # Stop execution after displaying warning

            st.info("Iniciando revisión ortográfica y gramatical. Esto puede tomar un momento...")
            with st.spinner("⏳ Realizando revisión..."):
                try:
                    ...
                    if matches:
                        ...
                        st.session_state.last_highlighted_html = highlighted_html # Store highlighted HTML
                        st.markdown(f"<div style='border: 1px solid #ddd; padding: 10px; border-radius: 5px; overflow-wrap: break-word; background-color: #fefefe; color: #333; max-height: 300px; overflow-y: auto;'>{highlighted_html}</div>", unsafe_allow_html=True)
                        st.success("🎉 ¡Revisión completada! Errores resaltados en rojo.")
                        st.session_state.word_download_saved_to_local = False # Mark as not yet saved (will be saved below)

                    else:
                        st.success("🎉 ¡No se encontraron errores ortográficos o gramaticales! Tu texto está impecable.")
                        st.session_state.last_highlighted_html = "" # No errors, so no highlighted HTML to store
                        st.session_state.word_download_saved_to_local = False # No doc to save
                except language_tool_python.utils.LanguageToolError as lt_error:
                    st.error(f"🚫 Error de LanguageTool: {lt_error}. Esto podría deberse a problemas de conexión o límites de la API pública. Por favor, **espera unos minutos e intenta de nuevo**, o considera ejecutar un servidor local de LanguageTool para un uso intensivo.")
                    st.session_state.last_highlighted_html = ""
                    st.session_state.word_download_saved_to_local = False
                except Exception as e:
                    st.error(f"🚫 Ocurrió un error inesperado al realizar la revisión: {e}. Por favor, inténtalo de nuevo más tarde.")
                    st.session_state.last_highlighted_html = "" # Clear in case of error
                    st.session_state.word_download_saved_to_local = False # No doc to save
            st.rerun() # Rerun to display the results or errors immediately

        # This block displays the last successful highlighting and download button
        # It's outside the "if st.button" so it persists across reruns.
        if st.session_state.last_highlighted_html.strip():
            st.subheader("✨ Último Resultado de Revisión:")
            st.markdown(f"<div style='border: 1px solid #ddd; padding: 10px; border-radius: 5px; overflow-wrap: break-word; background-color: #fefefe; color: #333; max-height: 300px; overflow-y: auto;'>{st.session_state.last_highlighted_html}</div>", unsafe_allow_html=True)
            st.info("Revisión previamente realizada. Haz clic en 'Revisar Texto' para volver a procesar.")

            # Generate DOCX bytes when the page loads IF there is highlighted HTML
            # Ensure 'archivo' is not None if we need its name for saving
            if archivo: # Use the currently uploaded 'archivo' if available, otherwise assume a name
                download_file_name_base = os.path.splitext(archivo.name)[0]
            else: # Fallback if 'archivo' is None (e.g., after a rerun without re-uploading)
                download_file_name_base = "texto_revisado"

            try:
                docx_bytes_for_download = create_word_doc_from_highlighted_html(st.session_state.last_highlighted_html)
                download_file_name = f"{download_file_name_base}_revisado.docx"

                st.download_button(
                    label="📥 Descargar Word con Errores Resaltados",
                    data=docx_bytes_for_download,
                    file_name=download_file_name,
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    key="download_highlighted_word_doc"
                )
                
                # Automatically save the generated DOCX to local history
                if not st.session_state.word_download_saved_to_local:
                    if save_file_to_local_disk_and_update_metadata(docx_bytes_for_download, f"{download_file_name_base}_revisado", ".docx", "revision_word_doc"):
                        st.session_state.word_download_saved_to_local = True # Mark as saved
                    else:
                        st.session_state.word_download_saved_to_local = False # Failed to save
            except Exception as e:
                st.error(f"Error al preparar el documento Word para descarga: {e}")


    else:
        st.info("☝️ Sube un archivo (PDF o Word) para iniciar la revisión ortográfica y gramatical.")
        st.session_state.last_extracted_text = "" # Ensure this is cleared if no file
        st.session_state.last_highlighted_html = "" # Ensure this is cleared if no file/text
        st.session_state.word_download_saved_to_local = False
    ...
    st.write("Sube tu archivo y conviértelo al formato deseado.")
    ...
    if uploaded_file_converter is not None:
        ...
        if file_name.endswith(".pdf"):
            ...
            # Mensaje actualizado para reflejar que solo es conversión de texto
            st.write("Convertirá tu documento Word a PDF extrayendo solo el texto (no requiere MS Word).")
            
            if st.button("Convertir a PDF", key="convert_docx_to_pdf"):
                with st.spinner(f"Convirtiendo DOCX a PDF (solo texto)..."):
                    texto_doc = ""
