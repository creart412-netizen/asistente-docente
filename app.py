...
# Establece la variable de entorno TESSDATA_PREFIX para que Tesseract encuentre los archivos de idioma
# ASEGÚRATE de que esta ruta sea la CORRECTA donde se encuentra tu carpeta 'tessdata'
...
st.set_page_config(page_title="Asistente Docente", page_icon="📚", layout="centered")
...
# Esta función es llamada *antes* de que el st.download_button se prepare.
...
# ==================== NAVEGACIÓN (Botones como Pestañas) ====================
tabs_names = [
    "✏️ Revisión Ortográfica y Gramatical",
    "📂 Generador de Exámenes y Plan de Clases",
    "🔢 Generador de Problemas Matemáticos", # NUEVA PESTAÑA
    "💬 Chat Educativo",
    "🔄 Convertidor de Documentos",
    "🎮 Juego Educativo", 
    "🗄️ Mis Archivos Guardados (Local)"
]

if "selected_section" not in st.session_state:
    st.session_state.selected_section = tabs_names[0]

cols = st.columns(len(tabs_names))

for i, tab_name in enumerate(tabs_names):
    with cols[i]:
        # Simple button logic for navigation without complex CSS for selected state
        if st.button(tab_name, key=f"nav_button_{i}"):
...
if st.session_state.selected_section == "✏️ Revisión Ortográfica y Gramatical":
    st.header("Corrección de Textos")
...
elif st.session_state.selected_section == "📂 Generador de Exámenes y Plan de Clases":
    ...
    if archivo_tema is not None:
        ...
        elif archivo_tema.lower().endswith(".docx"): # Corrected to use archivo_tema.name.lower()
            try:
        ...
        if texto_tema.strip():
            ...
            opcion = st.radio("¿Qué quieres generar?", ["Examen", "Plan de Clases"], key="tipo_generacion")
                ...
                if not openai.api_key:
                    ...
                    with st.spinner("⏳ Generando contenido con IA... Esto puede tardar unos segundos."):
                        ...
                        try:
                            ...
                            base_name_for_file = f"{opcion.replace(' ', '_')}_{nivel.replace(' ', '_')}"
                ...
                if st.session_state.generated_doc_display_type == "examen":
                    ...
                    for block in parsed_content:
                        if block["type"] == "question_answer":
                            ...
                            st.write("\n".join(block["content"]))
                elif st.session_state.generated_doc_display_type == "plan de clases":
...
elif st.session_state.selected_section == "🔢 Generador de Problemas Matemáticos": # NUEVA SECCIÓN
...
elif st.session_state.selected_section == "🔄 Convertidor de Documentos":
...
# ==================== NUEVA SECCIÓN: JUEGO EDUCATIVO ====================
elif st.session_state.selected_section == "🎮 Juego Educativo":
    ...
    game_selection = st.radio(
        ...
        ["Juego Externo: Math Slither"], # Solo esta opción
...
elif st.session_state.selected_section == "🗄️ Mis Archivos Guardados (Local)":
